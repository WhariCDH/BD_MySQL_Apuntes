...
/*Insertar invocación*/
INSERT INTO invocaciones VALUES
(1, 1, '2024-01-10', 300),
(2, 26, '2024-02-15', 3000),
(3, 30, '2024-03-01', 3000),
(4, 36, '2024-03-10', 3000),
(5, 51, '2024-03-15', 3000),
(6, 65, '2024-03-20', 3000);

/***********************/
/*1. SUBCONSULTA con IN*/
/***********************/

#Personajes que han sido invocados
SELECT nombre
FROM personajes
WHERE id_personaje IN (
    SELECT id_personaje FROM invocaciones
);

/*SUBCONSULTA con EXISTS* (mismo resultado con IN pero diferente logica)*/
SELECT nombre
FROM personajes p
WHERE EXISTS (
    SELECT 1
    FROM invocaciones i
    WHERE i.id_personaje = p.id_personaje
);
...
SELECT 
fecha_invocacion,
YEAR(fecha_invocacion) AS año,
MONTH(fecha_invocacion) AS mes,
DAY(fecha_invocacion) AS dia
FROM invocaciones;

/*Diferencia de días (DATEDIFF)*/
#Hace cuantos dias lo obtuve
SELECT 
id_personaje,
DATEDIFF(CURDATE(), fecha_invocacion) AS dias_desde_invocacion
FROM invocaciones;

/*Sumar días (DATE_ADD)*/
SELECT 
fecha_invocacion,
DATE_ADD(fecha_invocacion, INTERVAL 30 DAY) AS siguiente_evento
FROM invocaciones;
...
#CONCAT
SELECT 
CONCAT(nombre, ' - Nivel ', nivel) AS descripcion
FROM personajes;
...
SELECT 
nombre,
SUBSTRING(nombre, 1, 3) AS abreviatura
FROM personajes;

#TRIM (Desaparece los espacios sobrantes)
SELECT TRIM(nombre) AS Desaparecer_Espacios_excente FROM personajes;
...
SELECT 
nombre,
CAST(nivel AS CHAR) AS nivel_texto
FROM personajes;

#Convertir fecha a texto (con formato real)
SELECT 
nombre,
DATE_FORMAT(i.fecha_invocacion, '%d/%m/%Y') AS fecha_bonita
FROM personajes p
JOIN invocaciones i
ON p.id_personaje = i.id_personaje;

/*************************************/
/*7. EJERCICIO COMPLETO (MEZCLA TODO)*/
/*************************************/
SELECT 
p.nombre,
e.nombre_elemento,
DATE_FORMAT(i.fecha_invocacion, '%d-%m-%Y') AS fecha,
CONCAT('Nivel ', p.nivel) AS nivel_texto
FROM personajes p
